...
export class CartService {
  ...
  private readonly getCartQuery = `query ($id: ID!) { getCart(id: $id) { cart { id cost { subtotal { value currency } tax { value } shipping { value } total { value } } buyerIdentity { firstName lastName address1 address2 city provinceCode countryCode postalCode email phone } stores { ... on AmazonStore { errors { code message details { productIds } } store cartLines { quantity product { id } } offer { errors { code message details { ... on AmazonOfferErrorDetails { productIds } } } subtotal { value displayValue currency } margin { value displayValue currency } notAvailableIds selectedShippingMethod { id label price { value displayValue currency } taxes { value displayValue currency } total { value displayValue currency } } shippingMethods { id label price { value displayValue currency } taxes { value displayValue currency } total { value displayValue currency } } } } ... on ShopifyStore { errors { code message details { variantIds } } store cartLines { quantity variant { id } } offer { errors { code message details { ... on ShopifyOfferErrorDetails { variantIds } } } subtotal { value displayValue currency } margin { value displayValue currency } notAvailableIds selectedShippingMethod { id label price { value displayValue currency } taxes { value displayValue currency } total { value displayValue currency } } shippingMethods { id label price { value displayValue currency } taxes { value displayValue currency } total { value displayValue currency } } } } } } errors { code message } } }`;
  ...
  private async request(options: RequestInit) {
    const authOptions = await this.authService.makeRequestOptions();
   ...
   */
  public async createCart(variantId: string): Promise<any> {
   ...
   * @returns {GetCartQueryResult} - the cart data and any potential errors.
   */
  public async getCart(cartId: string): Promise<any> {
    const rawResponse = await this.request({
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
